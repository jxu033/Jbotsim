...
public class Topology extends _Properties implements ClockListener{
    ...
    */
    ...
    public void postComputeHandler() throws Exception {
        for (Robot robot : this.robots) {
            for (Robot robot1 : this.robots) {
                if ( robot.getID() != robot1.getID() && robot.mayCollide(robot1)) {
                    throw new Exception("Robot " + robot.getID()
                        + " may collide with Robot " + robot1.getID()
                            + "\n" + "Robot1 x: " + robot.getLocation().getX()
                            + " y: " + robot.getLocation().getY()
                            + " destination: x " + ((Destination2D)robot.dest).dest.getX()
                            + " y: " + ((Destination2D)robot.dest).dest.getY()
                            + "\n" + "Robot2 x: " + robot1.getLocation().getX()
                            + " y: " + robot1.getLocation().getY()
                            + " destination: x " + ((Destination2D)robot1.dest).dest.getX()
                            + " y: " + ((Destination2D)robot1.dest).dest.getY());
                }
            }
        }
    }
